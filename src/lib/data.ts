...
export function analyseData(data): {
	...
	const moodImg =
		data[0].priceUsd < data[data.length - 1].priceUsd
			? data[data.length - lastSegment.length ?? 1].priceUsd <
			  lastSegment[lastSegment.length - 1].priceUsd
				? MOOD_IMAGE.HAPPY
...
export function renderData(svg, xScale, yScale, line, { data, lines, skyline, moodImg }) {
...
function renderHills(line, xScale, yScale, hills, lines, skyline) {
	hills
